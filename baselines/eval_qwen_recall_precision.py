#!/usr/bin/env python3
"""
Qwen3-8B baseline: Exact-class retrieval from a pool of 1000 products.

For each query the model is shown 1000 product titles (ESCI candidates +
random distractors) and asked to select exactly N products, where N is the
number of Exact (E-labeled) products for that query.

Metrics (per-query, then averaged):
  Precision  = |selected ∩ E-class| / N
  Recall     = |selected ∩ E-class| / |E-class|
  F1         = harmonic mean of precision and recall
  Latency    = time-to-first-token in milliseconds (from vLLM RequestMetrics)

Only product titles are shown (truncated to 80 chars) so that 1000 products
fit inside the 32 K context window (~20 K tokens for the product list).

Usage
  python eval_qwen_recall_precision.py --num-queries 500 --output results/qwen_rp.csv
"""
...
def parse_selection(text: str, pool_size: int, n_select: int) -> list[int]:
    ...
    for n in numbers:
        ...
        if len(selected) == n_select:
            break
    return selected  # may be shorter than n_select if model under-generates
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--batch-size",      type=int, default=1,
                   help="Keep at 1 for accurate TTFT measurement")
...
def main():
    ...
    # One row per product event per query: TP / FP / FN — easy to filter for case studies
