#!/usr/bin/env python3
"""
Qwen3-8B baseline (full product info): Exact-class retrieval from a pool of 100 products.

Each product shows title, brand, color, bullet points, description (up to 500 chars each).
If the total prompt would overflow 32K tokens, the latter products' fields are truncated
progressively to stay within budget.

Metrics: Precision, Recall, F1, Latency (TTFT in ms).

Usage
  python eval_qwen_recall_precision_full.py --num-queries 500 --output results/qwen_rp_full.csv
"""
...
def build_message(grp: pd.DataFrame, pool: pd.DataFrame) -> list[dict]:
    ...
    for i, row in pool.iterrows():
        ...
        lines.append(entry)
        chars_used += len(entry) + 2  # +2 for the blank line separator
